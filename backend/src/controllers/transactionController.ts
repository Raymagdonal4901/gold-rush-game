...
// Claim Reward (Mining) — Dynamic Volatility Model
// คำนวณ yield ฝั่ง server ด้วย calculateDailyYield() แทนที่จะเชื่อค่า amount จาก frontend
export const claimReward = async (req: AuthRequest, res: Response) => {
    try {
        ...
        res.json({
            message: isJackpot ? '⚡ CRITICAL HIT! Claim successful' : 'Claim successful',
            amount,
            isJackpot,
