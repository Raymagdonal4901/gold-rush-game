...
export const LuckyDrawModal: React.FC<LuckyDrawModalProps> = ({ isOpen, onClose, user, onRefresh }) => {
  ...
  const handlePickCard = (index: number) => {
      ...
      // Simulate API Call delay and reveal
      setTimeout(() => {
          try {
              const res = MockDB.playLuckyDraw(user.id);
              if (res.success) {
                  setReward({ 
                      label: res.label, 
                ...
                {gameState === 'IDLE' && (
                    ...
                    >
                        {canPlayFree ? (
                            ...
                            <>
                                <span>จ่าย {LUCKY_DRAW_CONFIG.COST} บาท</span>
                            </>
                        )}
                    </button>
                )}
