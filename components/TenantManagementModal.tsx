...
export const GloveManagementModal: React.FC<GloveManagementModalProps> = ({
    ...
    const handleUpgrade = async () => {
        ...
        try {
            const res = await api.inventory.upgrade(equippedGlove.id);
            if (res.success) {
                setUpgradeMsg({
                    type: 'SUCCESS',
                    text: `UPGRADE SUCCESS!`,
                    level: res.item?.level, // api returns 'item' instead of 'newItem' based on other modals
                    subtext: 'เพิ่มประสิทธิภาพการทำกำไรเรียบร้อย'
                });
                onRefresh();
            } else {
                setUpgradeMsg({ type: 'ERROR', text: 'UPGRADE FAILED', subtext: res.message || 'วัตถุดิบไม่เพียงพอหรือเกิดข้อผิดพลาด' });
            }
        } catch (e: any) {
            setUpgradeMsg({ type: 'ERROR', text: 'UPGRADE FAILED', subtext: e.response?.data?.message || e.message });
        } finally {
                ...
                <div className="flex-1 flex flex-col items-center justify-center p-6 bg-[url('https://www.transparenttextures.com/patterns/dark-matter.png')]">
                    ...
                    {equippedGlove ? (
                        ...
                        >
                            ...
                            <span className="text-stone-500 font-bold group-hover:text-yellow-500">เลือกผู้จัดการ</span>
                            <span className="text-xs text-stone-600 mt-1">คลิกเพื่อจ้างงาน</span>
                        </div>
                    )}
                </div>
                            ...
                            <div className="bg-stone-950 border border-stone-800 rounded-xl p-4 relative overflow-hidden">
                                {upgradeReq ? (
                                    <>
                                        ...
                                        >
                                            {isUpgrading ? (
                                                <span className="animate-pulse flex items-center gap-2"><Hammer className="animate-bounce" size={16} /> กำลังเลื่อนขั้น...</span>
                                            ) : (
                                                <>
                                                    <ArrowUpCircle size={16} className="text-emerald-400 group-hover:text-white" />
                                                    เลื่อนขั้น (โอกาส {(upgradeReq.chance * 100).toFixed(0)}%)
                                                </>
                                            )}
                                        </button>
                                    </>
        ...
        return (
            <div className="flex flex-col h-full bg-stone-950">
                <div className="p-4 border-b border-stone-800 bg-stone-900 flex items-center gap-2">
                    ...
                    <span className="font-bold text-white">เลือกผู้จัดการ</span>
                </div>
                <div className="flex-1 overflow-y-auto custom-scrollbar p-4 grid grid-cols-2 gap-3">
                    {availableGloves.length === 0 ? (
                        <div className="col-span-2 text-center text-stone-500 py-10">ไม่มีการ์ดผู้จัดการในกระเป๋า</div>
                    ) : (
                ...
                <div className={`bg-stone-950 border border-stone-800 w-full max-w-sm rounded-2xl shadow-2xl overflow-hidden flex flex-col transition-all duration-300 ${view === 'SELECT' ? 'h-[600px]' : 'h-auto max-h-[90vh] overflow-y-auto custom-scrollbar'}`}>
                    ...
                    <div className="bg-stone-900 p-4 border-b border-stone-800 flex justify-between items-center shrink-0">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h2 className="text-lg font-display font-bold text-white">จัดการผู้จัดการห้อง</h2>
                                <p className="text-[10px] text-stone-500 uppercase tracking-wider">ประจำห้องพัก: {rig.name}</p>
                            </div>
                        </div>
