...
export const VIPModal: React.FC<VIPModalProps> = ({ isOpen, onClose, user }) => {
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/90 backdrop-blur-md p-4">
      <div className="bg-stone-950 border border-yellow-600/30 w-full max-w-4xl rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[85vh]">
        ...
        <div className="bg-gradient-to-r from-yellow-900/40 to-stone-900 p-6 border-b border-yellow-900/30 flex justify-between items-center shrink-0">
            <div className="flex items-center gap-4">
                ...
                <div>
                    <h2 className="text-3xl font-display font-bold text-white uppercase tracking-widest">VIP CLUB</h2>
                    <p className="text-sm text-yellow-500">ระดับปัจจุบัน: LV. {currentTier.level}</p>
                </div>
            </div>
        ...
        <div className="p-6 bg-stone-900 border-b border-stone-800">
            <div className="flex justify-between text-sm mb-2 text-stone-300">
                <span>ยอดสะสม: {currentExp.toLocaleString()} {CURRENCY}</span>
                {nextTier && <span>Next: {nextTier.minExp.toLocaleString()} {CURRENCY}</span>}
            </div>
            ...
            <p className="text-xs text-stone-500 mt-2 text-center">สะสมยอดใช้จ่ายจากการซื้อเครื่องจักรและอุปกรณ์เพื่อเลื่อนระดับ</p>
        </div>
                ...
                return (
                    ...
                    `}>
                        ...
                        <div className="flex-1">
                            ...
                            <p className="text-xs text-stone-500">ยอดสะสม {tier.minExp.toLocaleString()}+</p>
                        </div>
